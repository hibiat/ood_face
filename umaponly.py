"""
・学習済みモデルに対して、学習データ(既知クラスのみ)、テストデータ（既知クラス、未知クラス）を通したときのｆ
をUMAPで２次元に描画
"""
import argparse
import logging
...
test_ind_folders = 'oct_temp1' #既知クラス扱いにするフォルダ（テスト用）
test_ood_folders = 'oct_temp2' #未知クラス扱いにするフォルダ（テスト用）
...
dir_test = '/home/keisoku/work/ood2/data/oct/temp/'

#学習済みモデル
trainedmodel = "/home/keisoku/work/ood_face/out/20210210途中経過/out20210210/softmax/CP_best.pth"
backbone = 'resnet18'
metric = 'sortmax'
svalue = None #autoならNone,固定値ならその値を入れる
pretrain = True #pretrainありならTrueにする
fsize = 512 #512 #抽出する特徴量のサイズ
in_channels = 3 #入力チャンネル数
numclass = 4 #既知クラスの個数

#保存場所。モデルで判定したnpyもここに保存（２回以降の実行ではモデル判定をスキップできる）.モデルか使用データを変えた場合はnpyを消すこと。
savedir = os.path.join(os.path.dirname(trainedmodel), 'umap')
...
prune_rate = 0.1 #UMAPを作るときに使用するtrain dataの割合（間引き率）

#グラフ
is_dispeach = False #学習データ、テストデータのUMAP画像を単体で出力するならTrue
